...
void Light::add(std::initializer_list<std::initializer_list<GLfloat>> tupel) {
	...
	sources_and_colors_.push_back(glm::vec3(source_x, source_y, source_z));
	glm::vec3 rgb = hsv_to_rgb(wavelength_to_hue(wavelength), monochromaticity, amplitude);
	sources_and_colors_.push_back(rgb);
	num_new_lights_++;
}
...
std::vector<Uniform> Light::uniforms() {
	return std::vector<Uniform> {
		Uniform("light", sources_and_colors_.data(), [this](Uniform* u) {
			auto it = sources_and_colors_.end() - 2*num_new_lights_;
			glm::vec3* ptr = sources_and_colors_.data() + (it - sources_and_colors_.begin());
			u->update_and_grow(ptr, 2*num_new_lights_*sizeof(glm::vec3));
			num_new_lights_ = 0;
		}),
		Uniform("num_lights", [this](Uniform* u, Model* m) {
			u->update((int)(sources_and_colors_.size()/2));
		})
	};
}
